...
df<-read.csv("gen/output/adherance_df.csv")

# Run logistic Regression

df_Logistic1 <- glm(post_Compliant__c ~ analysis_email + analysis_call + n_case_communications + comm_timeframe, data = df)
summary(df_Logistic1)

df_Logistic2 <- glm(post_Compliant__c ~ analysis_call +  n_case_communications + comm_timeframe, data = df)
summary(df_Logistic2)

df_Logistic3 <- glm(post_Compliant__c ~ analysis_Letter + n_case_communications + comm_timeframe, data = df)
summary(df_Logistic3)

df_Logistic4 <- glm(post_Compliant__c ~ analysis_email + n_case_communications + comm_timeframe, data = df)
...
df_R2_CS1 <- 1-exp((df_Logistic1$deviance-df_Logistic1$null.deviance)/length(df))
# Nagelkerke R2_N= R2_CS/(1-exp(-(-2*LL0)/#obs))
df_R2_N1 <- df_R2_CS1/(1-(exp(-(df_Logistic1$null.deviance/length(df)))))

# Cox-Snell R2_CS=1-exp((-2*LL-(-2*LL0))/#obs), we use this as input to calculate Nagelkerke R2
# Nagelkerke R2 is our indication of % explained by variables
df_R2_CS2 <- 1-exp((df_Logistic2$deviance-df_Logistic2$null.deviance)/length(df))
# Nagelkerke R2_N= R2_CS/(1-exp(-(-2*LL0)/#obs))
df_R2_N2 <- df_R2_CS2/(1-(exp(-(df_Logistic2$null.deviance/length(df)))))

# Cox-Snell R2_CS=1-exp((-2*LL-(-2*LL0))/#obs), we use this as input to calculate Nagelkerke R2
# Nagelkerke R2 is our indication of % explained by variables
df_R2_CS3 <- 1-exp((df_Logistic3$deviance-df_Logistic3$null.deviance)/length(df$comm_id))
# Nagelkerke R2_N= R2_CS/(1-exp(-(-2*LL0)/#obs))
df_R2_N3 <- df_R2_CS3/(1-(exp(-(df_Logistic3$null.deviance/length(df$comm_id)))))

# Cox-Snell R2_CS=1-exp((-2*LL-(-2*LL0))/#obs), we use this as input to calculate Nagelkerke R2
# Nagelkerke R2 is our indication of % explained by variables
df_R2_CS4 <- 1-exp((df_Logistic4$deviance-df_Logistic4$null.deviance)/length(df$comm_id))
# Nagelkerke R2_N= R2_CS/(1-exp(-(-2*LL0)/#obs))
df_R2_N4 <- df_R2_CS4/(1-(exp(-(df_Logistic4$null.deviance/length(df$comm_id)))))
length(df$comm_id)
